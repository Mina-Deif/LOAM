...
class LoaderKITTI(Loader):
    def __init__(self, lidar_pcd, sequence):
        self.N_SCANS = 64
        #self.folder_path = os.path.join(dataset_path, 'dataset', 'sequences', sequence, 'velodyne')
        #self.pcds_list = os.listdir(self.folder_path)
        self.pcds_list = lidar_pcd
        #self.pcds_list.sort()
# all point clouds are loaded at the beggining 
#get one by one from airsim??
    ...
    def get_item(self, ind):
        ...
        #pcd = self.pcds_list.reshape(-1, 4)[:, :3]###############
        pcd = self.pcds_list.reshape(-1,3)
